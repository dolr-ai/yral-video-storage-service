...
docker exec -e PGPASSWORD="${POSTGRES_SUPERUSER_PASSWORD}" "${PATRONI_CONTAINER_ID}" \
  psql -U postgres -h 127.0.0.1 -v app_db_password="${APP_DB_PASSWORD}" \
  -c "CREATE USER storj WITH PASSWORD :'app_db_password';" 2>/dev/null || true

docker exec -e PGPASSWORD="${POSTGRES_SUPERUSER_PASSWORD}" "${PATRONI_CONTAINER_ID}" \
  psql -U postgres -h 127.0.0.1 -c "CREATE DATABASE video_fingerprint_index OWNER storj;" 2>/dev/null || true
