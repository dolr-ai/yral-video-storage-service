...
2. **Shard by `abs(hashtext(video_id)) % of = shard`** — even distribution; rejected video_id-range (skew risk).
...
```
When `shard = Some((of, idx))`, add `AND (abs(hashtext(v.video_id)) % $of) = $idx` to the WHERE clause (bind `of`, `idx`). `None` = no shard filter (current behavior). The existing `video_id` cursor + PK anti-join are unchanged: each shard still walks the whole `video_id` range (cheap, index-only) but the shard filter keeps only its 1/of of the missing rows, so the expensive download+ffmpeg work is partitioned with no overlap.
...
- Export the full `APP_VARS` (storage/DB/service creds — the same block currently gated to server_1) for all three nodes, since booting the app requires them (consts `.expect()` storage + service creds + `DATABASE_URL`).
- Add `PHASH_CONCURRENCY=4` to the app env on all three (sticky default; survives redeploy, unlike in-memory `config-set`).
...
- `videos_missing_canonical_phash` shard filter: seed N missing rows, call with `shard=Some((3, idx))` for idx 0/1/2, assert the three result sets are disjoint and their union = all N (every row lands in exactly one shard).
...
- `src/media_index/repo.rs` — `shard` param on `videos_missing_canonical_phash` (+ thread through any callers).
